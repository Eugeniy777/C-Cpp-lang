...
Начните с метода AddDocument, чтобы получился такой класс, как в примере. Кроме этого метода в классе должны быть поля, хранящие вектор документов и множество стоп-слов. Сделайте поля приватными. Названия полей остаются на ваше усмотрение, но сам класс надо назвать SearchServer, а его пока единственный метод — AddDocument.
