...
Пусть ваша поисковая система возвращает не любые документы, а максимум 5 документов с самой высокой релевантностью. Напишите функцию FindTopDocuments, которая должна находить не более MAX_RESULT_DOCUMENT_COUNT = 5 результатов. Объявите эту глобальную константу вверху своего решения.
...
Вызов функции FindTopDocuments поместите там же, где для вывода итогового результата мы вызывали FindDocuments. Саму FindDocuments замените на функцию FindAllDocuments, которая будет возвращать все результаты поиска в виде вектора пар из релевантности документа и его идентификатора {relevance, document_id}. Такой порядок следования элементов в паре облегчит сортировку пар.
...
Функция FindTopDocuments должна использовать функцию FindAllDocuments и возвращать документы, отсортированные по убыванию релевантности. Документы с одинаковой релевантностью должны быть упорядочены по убыванию id.
